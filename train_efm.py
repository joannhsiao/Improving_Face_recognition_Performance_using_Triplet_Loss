    ...
    def make_pairs(self, data_iter, pos_img, batch_size):
        ...
        for batch in data_iter:
            for i in range(batch_size):
                ...
                labels += [batch.label[0][i].asscalar()]
        #print(len(dataset))
    ...
    def __iter__(self):
        ...
        for i in range(self.length):
            ...
            for j in range(start, start+self.batch_size):
                data.append(self.batchs[j][0].asnumpy())
                label.append(self.labels[j].asscalar())
            for j in range(start, start+self.batch_size):
                data.append(self.batchs[j][1].asnumpy())
                label.append(self.labels[j].asscalar())
...
for epoch in range(280):
    ...
    for batch in data_train:
        ...
        with mx.autograd.record():              # record gradient of error
            ...
            """ loss function"""
            TL_loss = triplet_loss(anc/mx.nd.norm(anc), pos/mx.nd.nrom(pos), neg/mx.nd.nrom(neg))
    ...
    for batch in data_test:
        ...
        neg = mx.nd.array(neg).as_in_context(devs)
        """ loss function """
        TL_loss = triplet_loss(anc/mx.nd.nrom(anc), pos/mx.nd.nrom(pos), neg/mx.nd.nrom(neg))
        id_loss = softmax_cross_entropy(output[0: batch_size], label[0:batch_size])
        loss = id_loss + alpha * TL_loss
