...
class DataIter(mx.io.DataIter):
    ...
    def make_pairs(self, data_iter, pos_img, batch_size):
        ...
        for i in range(batch_size):
            ...
            dataset += [[batch[i].asnumpy(), pos_img[int(label[i].asscalar())].asnumpy(), batch[j].asnumpy()]]
            labels += [label[i].asscalar(), label[i].asscalar(), label[j].asscalar()]
        #print(len(dataset))
...
trainer = gluon.Trainer(net.collect_params(), 'sgd', {'learning_rate': lr, 'wd': 0.0001})
...
for epoch in range(200):
    ...
    print("Epoch {}: train loss {:g}, train acc {:g}, valid loss: {:g}, valid acc {:g}, in {:.1f} sec".format(
            epoch, train_loss/epoch_size, train_acc/epoch_size, valid_loss/(Testing_IMG_number/batch_size), 
            valid_acc/(Testing_IMG_number/batch_size), time.time()-tic), flush=True)
